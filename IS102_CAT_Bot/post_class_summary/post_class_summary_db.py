...
"""This method inserts the row_num from 1 into table 'left_post_class_question' """
def insert_row_num_q_id(r,q_id):
    connection = pymysql.connect(host = hostname,
                                user= username,
                                password=password,
                                db=database) 
    ...
    try:
        with connection.cursor() as cursor:
            # Create a new record
            sql = "INSERT INTO `left_post_class_question` (`row_num`, `question_id`) VALUES (%s, %s)"
            cursor.execute(sql, (r,q_id))
...
def retrieve_minrow_question_id():
    connection = pymysql.connect(host = hostname,
                                user= username,
                                password=password,
                                db=database) 
    ...
    try:
        with connection.cursor() as cursor:
            sql = "SELECT MIN(`row_num`)  FROM `left_post_class_question`"
            cursor.execute(sql)
            result_1 = cursor.fetchone()
            
            sql = "SELECT `question_id` FROM `left_post_class_question` WHERE `row_num` = %s "
            cursor.execute(sql,(result_1[0]))
...
def insert_post_class_answers(smu_email_id, question_id, question_hist, answer, group_id):
    ...
    try:
        with connection.cursor() as cursor:
            ...
            cursor.execute(sql, (smu_email_id, 1, question_id, question_hist, answer, group_id))
...
"""This method deletes the min row_num entry in 'left_post_class_question' table"""
def delete_min_row_qid(row_num):
    connection = pymysql.connect(host = hostname,
                                user= username,
                                password=password,
                                db=database)
    ...
    try:
        with connection.cursor() as cursor:
            sql = "DELETE FROM `left_post_class_question` WHERE `row_num` = %s "
...
"""This method retrieves the row_num only from 'left_post_class_question' table"""  
def retrieve_min_row():      
    connection = pymysql.connect(host = hostname,
                                user= username,
                                password=password,
                                db=database)
...
"""This method retrieves the q_id from `left_post_class_question` """
def retrieve_q_id(min_row_num):
    connection = pymysql.connect(host = hostname,
                                user= username,
                                password=password,
                                db=database)
    ...
    try:
        with connection.cursor() as cursor:
            ...
            sql = "SELECT `question_id` FROM `left_post_class_question` WHERE `row_num` = %s "
            cursor.execute(sql,(min_row_num))
            result = cursor.fetchone()
