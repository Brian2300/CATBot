...
$password = "password";
$hash = "$2y$10$9z5SNJuKP9BO3L7..U8GzeVNVeChCOiH.8aTEzpOZfis82SUVKnOy";
$demo = '$2y$10$VqOtb2B6xM1ODEWK2F8AZe6y7QLSpNTpVuXGvNTLyXX...';
	$sql = 'SELECT *
			FROM ' . USERS_TABLE . '
			WHERE user_id = 48';
		$result = $db->sql_query($sql);
		$user_row = $db->sql_fetchrow($result);
	//	echo $user_row[user_password ]; // return 48
	//	$re= $manager->detect_algorithm('$2y$10$9z5SNJuKP9BO3L7..U8GzeVNVeChCOiH.8aTEzpOZfis82SUVKnOy');
	//	echo ($re);
	    //echo (phpbb_hash('password')); // working, but different hash value
	
		
		$passwords_manager = $phpbb_container->get('passwords.manager');
		//echo ($passwords_manager->hash('password')); // hash value is keep changing
       // echo("   ***abc   ");
		//echo ($passwords_manager->check($password, $user_row[user_password]));
		//echo("   abc   ");
		if($passwords_manager->check($password, $hash)){
			//echo("match!!!!!!!! with hash");
		}
		if($passwords_manager->check($password, $hash)){
			//echo("match!!!!!!!! with demo");
		}
		//echo $manager -> hash('password','');
//$check1 = check($password,$hash, $user_row);
//echo $check1 +"hahahahahahahahahhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh";
// Start session
$user->session_begin();
$auth->acl($user->data);
